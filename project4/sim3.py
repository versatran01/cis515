...
def sim3_exp_SIM3(sim3):
    ...
    if np.isclose(lmda, 0.0) and np.isclose(theta, 0.0):  # both lambda = 0 and theta = 0
        ...
        v_1 = ((np.exp(lmda) - 1) / lmda) * np.eye(3)

        v_2 = ((theta * (1 - np.exp(lmda) * np.cos(theta)) + np.exp(lmda) * lmda * np.sin(theta)) / (
        ...
        v_3c = lmda * (np.exp(lmda) * np.cos(theta) - 1) / (theta ** 2 * (lmda ** 2 + theta ** 2))
        ...
        e_gamma = np.exp(lmda) * (
            np.eye(3) + (np.sin(theta) / theta) * omega + (
                (1 - np.cos(theta)) / theta ** 2) * omega * omega)  # now compute the final matrix in SIM3
